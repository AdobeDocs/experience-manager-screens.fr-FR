...
1. Créez l’agent de réplication Screens pour pub1. Par défaut, il existe un agent de réplication Screens qui pointe vers le port 4503. Celui-ci doit être activé.
1. Créez l’agent de réplication Screens pour pub2. Copiez l’agent de réplication Screens pour pub1 et modifiez le port afin qu’il pointe sur 4504 pour pub2.
...
1. **Liste autorisée des connecteurs de topologie** : à adapter aux adresses IP ou aux sous-réseaux couvrant les instances de publication. Veillez à placer sur la liste autorisée l’adresse IP/le nom d’hôte de toutes les instances de publication sans le numéro de port.
