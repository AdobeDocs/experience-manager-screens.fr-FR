...
Consultez la section [Exemple de stratégie JSON](#example-json) et un tableau décrivant l’utilisation des différentes [Attributs de stratégie](#policy-attributes).
...
>[!NOTE]
>Tous les appareils Android™ ont une `*sdcard*` dossier si un réel `*sdcard*` est inséré ou non. Une fois déployé, ce fichier se trouvera au même niveau que le dossier Downloads. Certains MDM tels que Samsung Knox peuvent utiliser l’emplacement de dossier *sdcard* comme *stockage interne*.
...
Vous pouvez attribuer un nom d’appareil convivial à votre lecteur Android™, envoyant ainsi le nom d’appareil attribué à AEM (Adobe Experience Manager). Cette fonctionnalité vous permet non seulement de nommer votre lecteur Android™, mais également d’attribuer facilement un contenu approprié.
