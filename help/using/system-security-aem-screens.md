...
# Considérations relatives à la sécurité du système pour AEM Screens {#security-checklist}
...
Le trafic du lecteur peut-il être redirigé vers un serveur malveillant et être invité à télécharger et lire du contenu multimédia malveillant ?

**Réponse**

C’est impossible, car la connexion HTTPS identifie les deux extrémités de la connexion et la chiffre. Si vous essayez d&#39;être au milieu et de l&#39;intercepter, vous ne voyez que du contenu chiffré. Si vous essayez d’emprunter l’identité du serveur, le lecteur vous refuse, car votre certificat est différent.
...
Utilisez le protocole HTTPS. C’est essentiel si vous vous préoccupez de sécurité. Avec le protocole HTTP, la communication est cryptée entre le lecteur et le serveur, et il est impossible d’intercepter le contenu ou de le modifier.
...
Si le hachage ne correspond pas, le logiciel tente de le revalider trois fois. Après trois tentatives, la commande de téléchargement est considérée comme non valide.
...
En supposant que vous soyez sur AMS, le logiciel s’occupe de toute la sécurité du serveur à l’aide des mêmes fonctionnalités que Sites ou Assets.
...
Placez les appareils hors de portée, de préférence dans un conteneur sécurisé, avec également un câblage sécurisé. Désactivez aussi les ports de télécommande infrarouge.
...
>[!NOTE]
>
>Il est recommandé d’équiper les appareils de fonctionnalités de mise à jour et de contrôle à distance correctes (bureau distant, solution MDM, etc.). Nous vous recommandons également d’utiliser un réseau privé, par exemple non exposé au Wi-Fi public.
...
1. compromettre le DNS pour emprunter l’identité du serveur sur ce nom d’hôte, et
1. compromettre
